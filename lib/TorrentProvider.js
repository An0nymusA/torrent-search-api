...
module.exports = class TorrentProvider {
  ...
  initCrawler() {
    this.scraperX = ScraperX();
    this.scraperX.setFilters(filters);
  }
  ...
  getUrl(category, query) {
    ...
    let url = this.baseUrl + (cat && cat.startsWith('url:') ? cat.substr(4) : this.searchUrl);
  ...
  getMagnet(torrent) {
    ...
    return this.scraperX(encodeURI(torrent.desc), this.magnetSelector);
  }

  getTorrentDetails(torrent) {
    if (this.torrentDetailsSelector) {
      return this.scraperX(encodeURI(torrent.desc), this.torrentDetailsSelector);
    }
    return Promise.resolve();
  }
  ...
  fetchAndParseUrl(url, limit) {
    return this.scraperX(url, this.itemsSelector, this.itemSelectors)
      .paginate(this.paginateSelector, limit)
  }
